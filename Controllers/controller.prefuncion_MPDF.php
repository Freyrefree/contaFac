...
function genera_pdf2($id, $datos){
  ...
  //================= datos del cliente
   ...
   $r_usocfdi = ""; 
    ...
    while($rowC = mysqli_fetch_array($resCl, MYSQLI_ASSOC)){ 
      ...
      $r_pais = utf8_encode($rowC["pais"]); 
      $r_estado = utf8_encode($rowC["estado"]); 
      $r_municipio = utf8_encode($rowC["municipio"]); 
      $r_colonia = utf8_encode($rowC["colonia"]); 
      $r_calle = utf8_encode($rowC["calle"]); 
  ...
  while($fila_emi = mysqli_fetch_array($res, MYSQLI_ASSOC)){
    ...
    $datos->set("regimen",$e_regimen_fiscal);
    $datos->buscaRegimen();
    $n_regimen_fiscal=$datos->get("descripcion");
    // if($e_regimen_fiscal=="621"){
    //   $n_regimen_fiscal="Incorporación Fiscal.";
    // }else{
    //   $n_regimen_fiscal="";
    // }
...
//      echo $cfdiComprobante['version']; 
      ...
      // $total = $cfdiComprobante['total']; //modifcado17112016
      $total = number_format($tot1,2,".",","); 

      //$subtotal = $cfdiComprobante['subTotal']; //modificado11172016
...
$taxid="";
$uso_cfdi="";//nuevo agregado ismael 271017 FACTURACION 3.3 
...
foreach ($xml->xpath('//cfdi:Comprobante//cfdi:Receptor') as $Receptor){
  ...
  $datos->set("uso_cfdi", addslashes(utf8_decode($uso_cfdi))); 
  $resuso= $datos->select_uso();

  while($fila_uso = mysqli_fetch_array($resuso, MYSQLI_ASSOC)){
     $uso_cfdi=$fila_uso["c_UsoCFDI"]." - ".utf8_encode($fila_uso["descripcion"]);
  }

   // $queryDatos2="SELECT * FROM llx_usocfdi WHERE uso_cfdi = '$uso_cfdi'"; 
   //  $resC2=mysqli_query($link, $queryDatos2);

   //  if ($resC2) {
   //    while($r2 = mysqli_fetch_array($resC2, MYSQLI_ASSOC)){
   //      $uso_cfdi=$r2["uso_cfdi"]." ".$r2["u_descripcion"];
   //    }
   // }
}
...
$base_16 = 0;
  ...
  while($row = mysqli_fetch_array($res_c, MYSQLI_ASSOC)){ 
        ...
        $unidad=$row['descripcion_empresa'];
        ...
        $base_16=$base_16 + $importe_translado;
        // =======================
        ...
        //=====================================
         $descripcionConcepto=$row['descripcionProd'];
        ...
        //===================================================
        // $taxr = number_format($taxr,2,".",","); 
        // $c_importe =number_format($importe_xml, 2, '.', '');
        ...
        //=============================================================
        // $p_descripcion_sat=utf8_encode($p_descripcion_sat);
        // $clave_unidad=utf8_encode($clave_unidad);
        ...
        $html_desc .= '<tr style="border-bottom: 1px solid #9e9e9e;">';
          ...
          $html_desc .= '<td HEIGHT="20" rowspan="2" align="center" style="font-size:10px; border-top: 1px solid #9e9e9e;  border-right: 1px solid #9e9e9e;  color: #505050">'.$unidad.'</td>';
          ...
          $html_desc .= '<td HEIGHT="20" align="center" style="font-size:10px; border-top: 1px solid #9e9e9e; border-right: 1px solid #9e9e9e;  color: #505050">'.$importe_t.'</td>';
          ...
          $html_desc .= '<td HEIGHT="20" align="center" style="font-size:10px; border-top: 1px solid #9e9e9e; border-right: 1px solid #9e9e9e;  color: #505050">'.$importe_r.'</td>';
        //fin nuevos agregado facturacion 3.3
...
foreach ($xml->xpath('//cfdi:Comprobante//cfdi:Impuestos//cfdi:Traslados//cfdi:Traslado') as $Traslado){ 
  ...
  if($base==""){    
    ...
    //if ($t_tasa=="0.160000") {//modificado ismael
    ...
    <HR style="height: 3px; background-color: #ccc;">
      ...
      <div style="width: 100%; border: 1pt solid #9e9e9e; border-radius:3pt; padding: 0pt; ">
                    ...
                    <tr>
                       <td>
                           <table width="100%" style="font-size:12px; font-family:Arial; border-collapse: collapse">
                             ...
                             <tr>
                                <td style="color: #505050"> <b style="color: #505050">Dirección: </b>'.utf8_encode($r_calle).', '.utf8_encode($r_int).''.utf8_encode($r_ext).'<br>
        ...
        $html =' <div style="width: 100%; border: 1pt solid #9e9e9e; border-radius:3pt; padding: 0pt; ">  
            <table border=0 style="border: 0px solid #000; border-collapse: collapse" width="100%">
              <tr>
                 <th  align="center" style="font-size:10px;color:#2745a4;background:#eae8e8;"><center><b style="color:#2745a4">Clave</b></center></th>

                  <th lign="center" style="font-size:10px;color:#2745a4;background:#eae8e8;"><center><b style="color:#2745a4">Cant.</b></center></th> 


                  <th lign="center" style="font-size:10px;color:#2745a4;background:#eae8e8;"><center><b style="color:#2745a4">Unidad</b></center></th>

                  <th  align="center" style="font-size:10px;color:#2745a4;background:#eae8e8;"><center><b style="color:#2745a4">Descripción</b></center></th>

                  <th align="left" style="font-size:10px;color:#2745a4;background:#eae8e8;" ><center><b style="color:#2745a4">Valor U.</b></center></th>

                  <th align="center" style="font-size:10px;color:#2745a4;background:#eae8e8;"><center><b style="color:#2745a4">Descuento</b></center></th>

                  <th align="center" colspan="2" style="font-size:10px;color:#2745a4;background:#eae8e8;"><center><b style="color:#2745a4">Impuestos</b></center></th>

                  <th align="center"  style="font-size:10px;color:#2745a4;background:#eae8e8;"><center><b style="color:#2745a4">Importe</b></center></th>
              </tr>
              <tr><td></td></tr>
              '.$html_desc.'
              <tr><td></td></tr>
...
// $mpdf=new mPDF('utf-8' , 'A4','', 4, 4, 4, 80, 12, 4);
// $mpdf=new mPDF('utf-8' , 'A4');
$mpdf->SetWatermarkText("PRE-FACTURA"); //Marca de agua
...
$mpdf->WriteHTML($html);
// $mpdf->WriteHTML($qr);
// $mpdf->Output("../comprobantes/".$UUID.".pdf");
// $mpdf->Output("../previsualizacion/".$id.".pdf");


$mpdf->Output("../tms/".$id.".pdf");
