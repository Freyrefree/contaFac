...
$rowid=$_POST['id'];////se tuienen todos loa ids seleccionados
...
function pruebaTimbrado($rowid, $datos){//modificado ismael FACTURACION 3.3. 
  ...
  */
  ...
  if($cliente->timbrar($cfdi, $opciones)){
    ...
    $fecha_factura = substr( date('c'), 0, 19);//nuevo agregado ismael 281117
    ...
    $datos->set("fecha_factura", addslashes(utf8_decode($fecha_factura))); 
    ...
    $datos->set("rfc_emisor", addslashes($_SESSION['fa_rfc']));
...
foreach ($row_list as $row_num) {
    ...
    $pagos_relacionados .= ' <pago10:Pago FechaPago="'.$fecha.'" FormaDePagoP="'.$forma_pago_actual.'" MonedaP="'.$moneda_pago_actual.'" Monto="'.$monto_pago_actual.'">';
    ...
    $pagos_relacionados .= ' </pago10:Pago>';

}//fin foreach nuevo ismael 271117
// $serie = "P";
// $folio = "1";
// $Serie $Folio
...
<cfdi:Comprobante  xsi:schemaLocation="http://www.sat.gob.mx/cfd/3 http://www.sat.gob.mx/sitio_internet/cfd/3/cfdv33.xsdhttp://www.sat.gob.mx/Pagos http://www.sat.gob.mx/sitio_internet/cfd/Pagos/Pagos10.xsd" xmlns:cfdi="http://www.sat.gob.mx/cfd/3" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:pago10="http://www.sat.gob.mx/Pagos" Version="3.3" Fecha="$fecha_actual" Sello="" NoCertificado="" Certificado="" SubTotal="$subtotal" Total="$total" $tipoDeComprobante LugarExpedicion="$cp" $Moneda $Serie $Folio>
